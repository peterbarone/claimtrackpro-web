...
export async function GET() {
  ...
  try {
    // Collection name inferred from schema relation: claims.claim_type -> collection claim_type
    const r = await fetch(
      `${DIRECTUS_URL}/items/claim_type?fields=id,name,code&sort=name`,
      {
    ...
    return NextResponse.json(
      { error: "Failed", detail: String(e?.message || e) },
      { status: 500 }
    );
  }
}

...
// Reuse same env + auth fallback pattern as other API routes
...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const res = await dx(`/items/contacts/${encodeURIComponent(id)}?fields=${encodeURIComponent(FIELDS)}`);
    const data = await res.json().catch(() => ({}));
    if (!res.ok) return NextResponse.json({ error: 'Failed to fetch contact', detail: data }, { status: res.status || 500 });
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const res = await dx(`/items/contacts/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(payload) });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) return NextResponse.json({ error: 'Failed to update contact', detail: data }, { status: res.status || 500 });
    return NextResponse.json({ data: (data as any).data });
  } catch (err: any) {
    return NextResponse.json({ error: String(err?.message || err) }, { status: 500 });
  }
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    const id = params.id;
    if (!id) return NextResponse.json({ error: 'Missing id' }, { status: 400 });
    const res = await dx(`/items/contacts/${encodeURIComponent(id)}`, { method: 'DELETE' });
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
      return NextResponse.json({ error: 'Failed to delete contact', detail: data }, { status: res.status || 500 });
    }

...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  try {
    const { id } = params; if (!id) return NextResponse.json({ error: 'Missing id' }, { status: 400 });
    const res = await dx(`/items/addresses/${encodeURIComponent(id)}?fields=${encodeURIComponent(FIELDS)}`);
    const data = await res.json().catch(() => ({}));
    if (!res.ok) return NextResponse.json({ error: 'Failed to fetch address', detail: data }, { status: res.status || 500 });
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const res = await dx(`/items/addresses/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(payload) });
    const data = await res.json().catch(() => ({}));
    if (!res.ok) return NextResponse.json({ error: 'Failed to update address', detail: data }, { status: res.status || 500 });
    return NextResponse.json({ data: (data as any).data });
  } catch (err: any) { return NextResponse.json({ error: String(err?.message || err) }, { status: 500 }); }
}
